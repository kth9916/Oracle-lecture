...
--   데이터 형식을 일치 시키시오. 
select trunc(sysdate - to_date(20220101, 'YYYYMMDD')) as 총날짜수
from dual;

--5-1. 자신이 태어난 날짜에서 현재까지 몇 일이 지났는지 출력 하세요.
select trunc(sysdate - to_date(19920502, 'YYYYMMDD')) as 총날짜수
from dual;
 ...
 */
 select eno, rpad(substr(eno,1,2),4,'*') as "가린 번호", ename, rpad(substr(ename,1,1),4,'*') as "가린 이름"
 from employee;
 
 -- 훨씬 더 세련되었고 다른 부분에도 적용이 가능하다.
  select eno, rpad(substr(eno,1,2),length(eno),'*') as "가린 번호", ename, rpad(substr(ename,1,1),length(ename),'*') as "가린 이름"
 from employee;
 ...
 9. 주민번호를 출력하되 801210-1****** 출력 하도록, 전화 번호 : 010-11******
	dual 테이블 사용
*/
select rpad(substr('801210-1000000',1,8),length('801210-1000000'),'*') as "가린 주민번호", 
        rpad(substr('010-1100-0000',1,6),length('010-1100-0000'),'*') as "가린 전화번호"
from dual;
